...
@pytest.mark.parametrize("obj",
                         [(CombatMech()),
                          ...
                          (Mountain()),
                          (Building(health=1))])
def test_kill_object(obj):
    board = Board()

    kill_pos = (0, 0)

    board[kill_pos] = GroundTile(_object=obj)
    orig_tile = copy.deepcopy(board[kill_pos])

    board.fill_object_position_cache()
    attack = Attack(attacker=kill_pos, weapon=Kill(), vector=None)
    original_tiles = apply_attack(board, attack)
    assert len(original_tiles) == 1

    assert orig_tile == original_tiles[kill_pos]
    assert obj.get_health() == 0


def test_kill_attack_exploding_vek_chain_reaction():
    board = Board()
    ...
    board.fill_object_position_cache()
    vek0_attack = Attack(attacker=spiderling0_pos, weapon=Kill(),
                         vector=None)
...
def test_kill_on_shell_psion():
    ...
    attack = Attack(attacker=shell_psion0_pos, weapon=Kill(),
                    vector=None)
...
@pytest.mark.parametrize("obj",
                         [(Firefly()),
                          (CombatMech())])
def test_kill_object_with_shell_psion_present(obj):
    board = Board()
    ...
    attack = Attack(attacker=obj0_pos, weapon=Kill(),
                    vector=None)
...
@pytest.mark.parametrize("obj",
                         [(Firefly()),
                          (CombatMech())])
def test_kill_on_shielded_object(obj):
    board = Board()
...
def test_kill_on_shielded_building():
    board = Board()
